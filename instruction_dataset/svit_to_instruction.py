    ...
    for item in tqdm.tqdm(input_data):
        ...
        for conversation in item["conversations"]:
            if len(conversation["content"]) == 2:  # single turn
                ...
                output_data.append({"input": instruction, "output": answer["value"]})
            else:
                ...
                for i, message in enumerate(conversation["content"]):
                    if message["from"] == "user":
                        ...
                        if i == 1:
                            ...
                            output_item = {
                                "input": f"<img_path>{img_path}<img_path>"
                                + first_instruction,
                                "output": output,
                            }
...
if __name__ == "__main__":
    ...
    image_dir = (
        "/cpfs/shared/research-llm/instruc_data_en/multimodal_instruct_tuning/SVIT/raw"
    )
    os.makedirs("converted_datasets/svit", exist_ok=True)

    images = json.load(
        open(
            "/cpfs/shared/research-llm/instruc_data_en/multimodal_instruct_tuning/SVIT/raw/annotations/vg/image_data.json",
            "r",
        )
    )
    id2path = {
        image["image_id"]: "VG_100K" + image["url"].split("VG_100K")[1]
        for image in images
    }

    llava_to_instruction(
        "/cpfs/shared/research-llm/instruc_data_en/multimodal_instruct_tuning/SVIT/data/svit.json",
        "converted_datasets/svit/svit_full.json",
    )
