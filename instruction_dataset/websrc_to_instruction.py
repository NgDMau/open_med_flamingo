                    ...
                    if mode == "qa":
                        ...
                        if random.random() < 0.5:
                            instruction = (
                                ...
                                + last["question"]
                                + " "
                                + random.choice(direct_answer_instructions)
                            )
                        else:
                            instruction = (
                                question_prompt
                        ...
                        if random.random() < 0.1:
                            with open(
                                f"{d}/processed_data/{last['id'][2:9]}.html", "r"
                            ) as html_file:
...
def dataset_split(args, data, mode):
    ...
    for data_point in data:
        ...
        domain_website = (
            data_point["input"].split("/")[-4][:2] + data_point["input"].split("/")[-3]
        )
        ...
        if domain_website in split["test"]:
            ...
            raise ValueError(
                f"The domain_website {domain_website} is not in the split dict {split}. data_point: {data_point}"
            )

    # Write the train, test, and dev datasets to JSON files
    with open(
        osp.join(
            "converted_datasets/websrc", f"websrc_{mode}_train" + args.suffix + ".json"
        ),
        "w",
    ) as f:
    ...
    with open(
        osp.join(
            "converted_datasets/websrc", f"websrc_{mode}_test" + args.suffix + ".json"
        ),
        "w",
    ) as f:
    ...
    with open(
        osp.join(
            "converted_datasets/websrc", f"websrc_{mode}_dev" + args.suffix + ".json"
        ),
        "w",
    ) as f:
