...
alpaca_cot_dir = (
    "/cpfs/shared/research-llm/instruc_data_en/language_only_instruct_tuning/Alpaca-CoT"
)
...
def main():
    ...
    for file_name in file_list:
        ...
        file_data = [
            ...
            if len(item["input"]) < 512 and len(item["output"]) < 1024
        ]
        json.dump(
            file_data,
            open(f"converted_datasets/alpaca_cot/{os.path.basename(file_name)}", "w"),
            indent=4,
        )
    ...
    json.dump(
        data,
        open(
            f"converted_datasets/alpaca_cot/alpaca_cot_merged-{int(len(data)/1000)}k.json",
            "w",
        ),
        indent=4,
    )
