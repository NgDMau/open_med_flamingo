...
def gqa_to_alpaca_format(scene_graphs, questions, truncate=None):
    ...
    for question_id in question_ids:
        ...
        if random.random() > 0.5:
            instruction = (
                question_text + " " + random.choice(direct_answer_instructions)
            )
        else:
            instruction = (
                random.choice(direct_answer_instructions) + " " + question_text
            )
...
questions = json.load(
    open(
        "/cpfs/user/chendelong/instruction_tuning_dataset/gqa/val_balanced_questions.json"
    )
)
...
questions = json.load(
    open(
        "/cpfs/user/chendelong/instruction_tuning_dataset/gqa/train_balanced_questions.json"
    )
)
