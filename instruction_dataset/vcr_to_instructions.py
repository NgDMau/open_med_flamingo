        ...
        if i in mentioned_objects:
            ...
            if f"{obj}-{map_to_letters(i)}" in instruction:
                instruction = instruction.replace(
                    f"{obj}-{map_to_letters(i)}",
                    f"{obj}-{map_to_letters(i)}<img_path>{cropped_image_path}<img_path>",
                )
            else:
                identifier_to_region_image[f"{obj}-{map_to_letters(i)}"] = (
                    cropped_image_path
                )
...
def generate_output(
    object_names, answer_choices, answer_label, rationale_choices, rationale_label
):
    ...
    for inds in range(len(rationale_choices[rationale_label])):
        if type(rationale_choices[rationale_label][inds]) == list:
            identifier, mentioned_objects = replace_object_inds(
                rationale_choices[rationale_label][inds],
                object_names,
                return_mentioned_objects=True,
            )
...
def process_vcr_split(split):
    instructions = []
    with open(
        f"/cpfs/user/chendelong/instruction_tuning_dataset/vcr/{split}.jsonl"
    ) as f:
    ...
    for item in tqdm.tqdm(data):
        ...
        with open(
            os.path.join(
                "/cpfs/user/chendelong/instruction_tuning_dataset/vcr/vcr1images",
                item["metadata_fn"],
            )
        ) as metadata_file:
        ...
        mentioned_objects = get_mentioned_objects(
        ...
        instruction = generate_instruction(
            ...
            answer_choices,
            mentioned_objects,
            full_image=f"/cpfs/user/chendelong/instruction_tuning_dataset/vcr/vcr1images/{image_filename}",
        )
        input_data = f"<img_path>/cpfs/user/chendelong/instruction_tuning_dataset/vcr/vcr1images/{image_filename}<img_path>"
        output, all_mentioned_objects = generate_output(
            object_names,
            answer_choices,
            answer_label,
            rationale_choices,
