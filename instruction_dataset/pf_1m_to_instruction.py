...
for sample in samples:
    ...
    instruction = instruction.replace(
        "/cpfs/shared/research/multimodal_instruct_tuning",
        "/cpfs/shared/research-llm/instruc_data_en/multimodal_instruct_tuning",
    )

    converted_samples.append({"input": instruction, "output": sample["output"]})

# 按照'reward_score'字段进行排序
sorted_samples = sorted(
    converted_samples, key=lambda x: x.get("reward_score", 0), reverse=True
)
