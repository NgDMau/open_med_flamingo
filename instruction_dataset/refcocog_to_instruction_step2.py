...
for i, sample in tqdm(enumerate(data), total=len(data), desc="Processing images"):
    # 提取bbox信息
    bbox_info = sample["input"][
        sample["input"].find("{") : sample["input"].rfind("}") + 1
    ]
    bbox = json.loads(bbox_info.replace("'", '"'))

    # 读取input中对应的图片
    image_path = sample["input"][
        sample["input"].find("<img_path>") + 10 : sample["input"].rfind("<img_path>")
    ]
    ...
    cropped_image = image.crop(
        (
            bbox["bbox x axis"],
            bbox["bbox y axis"],
            bbox["bbox x axis"] + bbox["bbox width"],
            bbox["bbox y axis"] + bbox["bbox height"],
        )
    )
    ...
    sample["input"] = (
        sample["input"]
        .replace(bbox_info, f"<img_path>{new_image_path}<img_path>")
        .replace("  ", " ")
    )
