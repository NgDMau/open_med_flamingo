        ...
        for i, conversation in enumerate(item["conversations"]):
            if conversation["from"] == "human":
                ...
                if i == 1:
                    ...
                    first_instruction = first_instruction.replace(
                        "<image>\n", ""
                    ).replace("\n<image>", "")
                    output_item = {
                        "input": f"<img_path>{coco_dir}/COCO_train2014_{item['image']}<img_path>"
                        + first_instruction,
                        "output": output,
                    }
...
if __name__ == "__main__":
    ...
    llava_to_instruction(
        ...
        "converted_datasets/llava/complex_reasoning_77k.json",
    )
    llava_to_instruction("detail_23k.json", "converted_datasets/llava/detail_23k.json")
    llava_to_instruction(
        "conversation_58k.json", "converted_datasets/llava/conversation_58k.json"
    )
