...
def read_coco_annotations_captions(annotations_file):
    ...
    # Create a dictionary for easy lookup of bbox by annotation_id
    caption_dict = {
        anno["image_id"]: anno["caption"] for anno in coco_data["annotations"]
    }

    return caption_dict


def read_refexp_data(
    json_path, img_root, image_size_dict, bbox_dict, caption_dict, split
):
    ...
    for annotation in annotations:
        ...
        img_path = os.path.join(
            img_root, f"COCO_{split}2014_{str(image_id).zfill(12)}.jpg"
        )
...
def bbox_to_str(x, y, w, h):
    return str({"bbox x axis": x, "bbox y axis": y, "bbox width": w, "bbox height": h})


def build_tasks(samples):
    tasks = []
    for sample in samples:
        task3_instruction = random.choice(task3_instructions).replace(
            "<REGION>", bbox_to_str(sample["x"], sample["y"], sample["w"], sample["h"])
        )
...
for split in ["train"]:
    ...
    image_size_dict = read_coco_image_size(
        f"/cpfs/user/chendelong/downloads/mscoco_2014/annotations/instances_{split}2014.json"
    )
    bbox_dict = read_coco_annotations(
        f"/cpfs/user/chendelong/downloads/mscoco_2014/annotations/instances_{split}2014.json"
    )
    caption_dict = read_coco_annotations_captions(
        f"/cpfs/user/chendelong/downloads/mscoco_2014/annotations/captions_{split}2014.json"
    )

    # Call the function with the desired file path and root image path
    samples = read_refexp_data(
        f"/cpfs/user/chendelong/instruction_tuning_dataset/refcocog/google_refexp_{split}_201511.json",
        f"/cpfs/user/chendelong/downloads/mscoco_2014/{split}2014",
        image_size_dict,
        bbox_dict,
        caption_dict,
        split=split,
    ...
    with open(
        f"converted_datasets/refcocog/refcocog_{split}_instruction.json", "w"
    ) as f:
