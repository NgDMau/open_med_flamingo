...
for subset in subsets:
    instruction_file = (
        "/cpfs/shared/research-llm/instruc_data_en/multimodal_instruct_tuning/MIMIC-IT/"
        + subset
        + ".json"
    )
    ...
    for instruction_id in tqdm.tqdm(list(instructions.keys())):
        ...
        if (
            "annotations" in instruction
        ):  # some responses have 'according to annotations...'
            continue
        if (
            "[male]" in output or "[female]" in output
        ):  # visual storytelling dataset tag
            continue
        if "VST" in subset:
            for image_id in image_ids:
        ...
        converted_sample = {"input": instruction, "output": output}
    ...
    json.dump(
        converted_samples,
        open(f'converted_datasets/mimic_it/{subset.split("/")[-1]}.json', "w"),
        indent=4,
    )
