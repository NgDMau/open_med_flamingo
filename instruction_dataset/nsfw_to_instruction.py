...
def generate_random_instruction():
    ...
    # 在内容中的随机位置插入 <image>
    content_to_insert = (
        content_to_insert[:position] + "<image>" + content_to_insert[position:]
    )
...
nsfw_images = get_images_with_keyword(
    "/cpfs/user/chendelong/instruction_tuning_dataset/data_augmentation/P2datasetFull/train/2",
    "train3",
)
...
class InstructionDataset:
    ...
    def __getitem__(self, idx):
        ...
        input_data, img_paths = extract_path_and_convert_token(
            sample["input"], self.image_dir_path
        )
...
for dataset_info in datasets_info:  # [i:i+1]:
    ...
    if "cpfs" not in dataset_info["json_path"]:
        dataset_info["json_path"] = (
            "/cpfs/user/chendelong/open_flamingo_v2/" + dataset_info["json_path"]
        )
    ...
    for i in range(dataset_info["ratio"] * 100):
        ...
        nsfw_sample = {
            "input": input_data.replace(
                "<image>", f"<img_path>{random.choice(nsfw_images)}<img_path>"
            ),
...
os.makedirs("converted_datasets/nsfw", exist_ok=True)
json.dump(
    all_samples, open("converted_datasets/nsfw/nsfw_in_context.json", "w"), indent=4
)
