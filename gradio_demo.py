...
def bot(
    ...
    inference_results, full_texts = inferencer(
        inputs,
        image_paths,
        max_new_token,
        num_beams,
    ...
    date = (
        datetime.now().astimezone(pytz.timezone("Asia/Shanghai")).strftime("%Y-%m-%d")
    )
...
def build_conversation_demo(title_markdown, note_markdown):
    with gr.Blocks(title="Clever Flamingo🦩") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion(
                    ...
                    num_beams_bar = gr.Slider(0.0, 10, 1, label="num_beams", step=1)
                ...
                with gr.Accordion(
                    ...
                    with gr.Row():
                        ...
                        user_prefix = gr.Text("Human", label="User Prefix")
                        seperator = gr.Text("\n### ", label="Seperator")
            ...
            with gr.Column(scale=6):
                with gr.Row():
                    with gr.Column():
                        chatbot = gr.Chatbot(
                            elem_id="chatbot", color_map=["blue", "pink"]
                        ).style(height=750)
                with gr.Row():
        ...
        clear_btn.click(
            clear, [state], [state, chatbot, textbox, imagebox, model_inputs]
        )
...
"""
    ...
    os.environ["GRADIO_TEMP_DIR"] = (
        "/cpfs/user/chendelong/open_flamingo_v2/serving/GRADIO_TEMP_DIR"
    )
