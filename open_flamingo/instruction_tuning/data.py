...
def extract_path_and_convert_token(input_data, img_dir):
    ...
    img_paths = [
        os.path.join(img_dir, path) for path in img_path_pattern.findall(input_data)
    ]

    # FIXME: temproary fix for reorganized cpfs
    img_paths = [
        img_path.replace(
            "research/multimodal_instruct_tuning",
            "research-llm/instruc_data_en/multimodal_instruct_tuning",
        )
...
def load_and_process_images(
    img_paths, zero_image, image_processor, max_img=None, is_train=True
):
    images = []
    ...
    if max_img is not None:
        if num_images > max_img:
            print(
                f"more than max_img={max_img} images, only use the first {max_img} images"
            )
...
class InstructionDataset(Dataset):
    def __init__(
        ...
        if (
            "json_path" not in self.configs[0].keys()
            and "input" in self.configs[0].keys()
        ):
            ...
            self.configs = [
                {
                    ...
                    "dataset_name": os.path.basename(config_json_path).replace(
                        ".json", ""
                    ),
                    "ratio": 1.0,
                }
        ...
        if num_samples > 0:
            ...
            #     weights = [ratio_weight * sqrt_weight for ratio_weight, sqrt_weight in zip(ratio_weights, sqrt_weights)]
                ...
                raise ValueError(
                    f'Invalid dataset_sampling_mode: {args.dataset_sampling_mode}. Must be "ratio", "sqrt"'
                )  # or "ratio*sqrt"')
            ...
            num_samples_per_dataset = [
                int(num_samples * weight) for weight in normalized_weights
            ]
            if sum(num_samples_per_dataset) < num_samples:
                random_dataset_index = random.randint(
                    0, len(num_samples_per_dataset) - 1
                )
                num_samples_per_dataset[random_dataset_index] += num_samples - sum(
                    num_samples_per_dataset
                )
        ...
        for i, config in enumerate(self.configs):
            if args.rank == 0:
                logger.info(
                    f"Loading dataset {i+1}/{len(self.configs)} from {config['json_path']}"
                )
            dataset_json_path = config["json_path"]
            img_dir = config["img_dir"]
            ...
            for sample in samples:
                ...
                if self.mode == "train":
                    if args.skip_check_overlength:
                        ...
                        instruction_converted, _ = extract_path_and_convert_token(
                            sample["input"], img_dir
                        )
                        input_tokenized_length = len(
                            self.tokenizer(
                        ...
                        if input_tokenized_length < max_length and sample[
                            "output"
                        ] not in ["", " ", "  ", "\n"]:
                            self.samples.append(sample)
                        else:
        ...
        if logger is not None and args is not None and args.rank == 0:
            logger.info(
                f'Instruction prompt templete: "{self.instruction_prompt_templete}"'
            )
            logger.info(
                f"Initialized instruction tuning dataset from {config_json_path}"
            )
            ...
            for i, config in enumerate(self.configs):
                logger.info(
                    f"Dataset [{i + 1}/{len(self.configs)}]: {config['dataset_name']}"
                )
                logger.info(f"\tConfig Sampling Ratio: [{config['ratio']}]")
                # logger.info(f"\tImage directory: {config['img_dir']}")
                logger.info(
                    f"\tNum samples (total available): {total_samples_per_dataset[i]} ({total_samples_per_dataset[i]/total_samples*100:.2f}%)"
                )
                if self.num_samples > 0:
                    logger.info(
                        f"\tNum samples (after sampling):  {sampled_counts[i]} ({normalized_weights[i]*100:.2f}%)"
                    )
                if self.mode == "train" and not args.skip_check_overlength:
                    logger.info(
                        f"\tNum over-length samples:       {over_length_counts[i] if i in over_length_counts.keys() else 0}"
                    )

        # Create and cache a zero-filled image for padding
        self.zero_image = torch.zeros_like(
            self.image_processor(Image.new("RGB", (224, 224)))
        )
    ...
    def __getitem__(self, idx):
        ...
        instruction, img_path = extract_path_and_convert_token(
            sample["input"], sample["img_dir"]
        )
        ...
        if self.mode == "train":
            ...
            input_ids, target_mask = self.multi_instrucions_tokenization(
            ...
            return (
                ...
                target_mask,
                torch.tensor(
                    dataset_idxs[: self.multiturn_augmentation], dtype=torch.long
                ),
            )

        elif self.mode == "test":
            instruction_str = get_prompt_instruction(
                instructions[0], self.instruction_prompt_templete
            )
            text = self.tokenizer(
                instruction_str,
                return_tensors="pt",
    ...
    def multi_instrucions_tokenization(self, instructions, targets):
        ...
        for i in range(len(instructions)):
            if i == 0:
                ...
                if (
                    self.instruction_prompt_templete == "guanaco"
                    or self.instruction_prompt_templete == "guanaco-no-prompt"
                ):
